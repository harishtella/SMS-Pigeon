...
=begin
  def poll_sms?(keyword)
    keyword_db_entry = Keyword.first(:conditions => {:value => keyword})
    if keyword_db_entry.nil? 
      return false
    elsif keyword_db_entry.owner_type.eql? "Poll"
      return true
    else 
      return false
    end
  ...
  def process_poll(phone, text, keyword)
    votes_string = text.split(" ")[1].strip
    votes = []
    votes_string.each_char {|x| votes << x.upcase }

    keyword_db_entry = Keyword.first(:conditions => {:value => keyword})
    poll = keyword_db_entry.owner
    questions = poll.questions

    current_user = User.for(phone) 

    unless (votes.size.eql? questions.size)
      if votes.size > questions.size
        return "Please answer no more than the given questions"
      else
    ...
    votes.each_with_index do |v, i|
      q = questions[i] 
      c = q.choices.first(:conditions => "keyword = '#{v}'")

      if c.nil? 
        return "Please use only then given choices to answer"
      end
